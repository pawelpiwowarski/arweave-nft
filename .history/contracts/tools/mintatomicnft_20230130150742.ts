...
import fs from 'fs';
import { configureWallet, warp } from "./configureServerEnv";
...
export const mintAtomicNFT = async (address: any, raw_data: any, name: any, descrition: any) => {
    ...
    console.log(contractTags)
    const { contractTxId } = await warp.register(tx.id, "node2");

  
    console.log(`Check the data: https://sonar.warp.cc/${contractTxId}`);

    return {contract: `https://sonar.warp.cc/#/app/contract/${contractTxId}`, image: `https://arweave.net/${contractTxId}`};
