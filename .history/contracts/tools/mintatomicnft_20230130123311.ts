...
export const mintAtomicNFT = async (address: any, raw_data: any, name: any, descrition: any) => {
    ...
    const uint8array = new Uint8Array(raw_data);
    const mimetype = await fileTypeFromBuffer(uint8array);
    ...
    const tx = await bundlr.upload(raw_data.toString() , { tags: contractTags });
